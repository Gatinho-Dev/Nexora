...
export function ChatArea({
  ...
  // Uma conversa aberta em uma aba em segundo plano continua não lida. O
  // cursor da conta só avança quando a janela volta a ficar realmente visível.
  useEffect(() => {
    ...
    <main className="flex-1 flex flex-col min-w-0 h-full bg-chat relative select-text">
      ...
      {showScrollBottom && (
        <button
          ...
          className="absolute bottom-16 right-6 z-20 flex items-center gap-2 rounded-full bg-primary px-3.5 py-2 text-xs font-semibold text-primary-foreground transition-colors hover:bg-primary/90"
        >
