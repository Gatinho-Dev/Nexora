export type KeybindAction = "mute" | "deafen" | "open" | "answer" | "hangup" | "overlay" | "streamer";
...
export function parseKeybinds(data: Record<string, unknown> | null | undefined): KeybindMap {
  const raw = data?.keybinds;
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return DEFAULT_KEYBINDS;
  const values = raw as Record<string, unknown>;
  return Object.fromEntries(Object.keys(KEYBIND_LABELS).map(action => [action, typeof values[action] === "string" ? values[action] : DEFAULT_KEYBINDS[action as KeybindAction]])) as KeybindMap;
}

export function duplicateKeybinds(keybinds: KeybindMap) {
  const seen = new Map<string, KeybindAction[]>();
  for (const [action, shortcut] of Object.entries(keybinds) as [KeybindAction, string | undefined][]) {
    if (!shortcut) continue;
    seen.set(shortcut, [...(seen.get(shortcut) ?? []), action]);
  }
  return new Set([...seen.entries()].filter(([, actions]) => actions.length > 1).map(([shortcut]) => shortcut));
}
